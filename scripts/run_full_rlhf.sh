#!/usr/bin/env bash
# Full-batch RLHF runs for all supervised checkpoints.
# Usage: bash scripts/run_full_rlhf.sh
...
total=${#models[@]}
start_time=$(date +%s)
log_dir="logs/rl_full_run"
mkdir -p "$log_dir"
output_root="RL/full_run"
mkdir -p "$output_root"
...
echo "\nAll runs dispatched. Check ${log_dir} for per-model logs and ${output_root} for checkpoints."
